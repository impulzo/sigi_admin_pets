...
## DOCKER ##
docker-image:
	docker build -t laravel-app-pets .
...
# 	example run: make docker-up network=network_name --> docker network ls || docker network inspect container_id
	docker run -d -p 8000:80 --name pets-app \
		--network $(network) \
...
docker-permissions:
	docker exec -it pets-app chown -R www-data:www-data /var/www/html/storage /var/www/html/bootstrap/cache
	docker exec -it pets-app chmod -R 775 /var/www/html/storage /var/www/html/bootstrap/cache
